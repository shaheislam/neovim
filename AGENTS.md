...
- Native idle handoff keeps the matching OpenCode terminal generation open, reloads changed buffers, fills quickfix without opening it, focuses the latest changed normal file, and, when the root `.plan.md` changed, opens Diffview before focusing one reused plan tab. A newer accepted route discards pending work from the prior route.
