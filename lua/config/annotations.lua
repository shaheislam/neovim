...
local post_instruction = "after resolving each annotation, edit the item from annotation file and prepend the annotation with 'RESOLVED: '"
...
local function current_buffer_path(bufnr)
  ...
  if diffview_path then return vim.fn.fnamemodify(diffview_path, ":p"), true end
...
local function repo_root(path)
  ...
  if vim.fn.isdirectory(start) ~= 1 then start = vim.fn.fnamemodify(start, ":h") end
...
local function annotation_filename(path, root, is_diffview)
  ...
  if filename == vim.fn.fnamemodify(path, ":p") then filename = relative_path(path, root) end

  if is_diffview then
    local worktree_filename = filename:match("^%.git/worktrees/[^/]+/[^/]+/(.+)$")
    if worktree_filename then return worktree_filename end

    local git_filename = filename:match("^%.git/[^/]+/(.+)$")
    if git_filename then return git_filename end
...
local function truncate(text)
  text = text:gsub("%s+", " ")
  if #text <= max_ghost_length then return text end
...
function M.refresh_all()
  for _, bufnr in ipairs(vim.api.nvim_list_bufs()) do
    if vim.api.nvim_buf_is_loaded(bufnr) then M.refresh_buffer(bufnr) end
  end
end
...
local function jump(direction)
  ...
  if not file then return end
  local line = math.max(vim.api.nvim_win_get_cursor(0)[1], 1)
  local matches = vim.tbl_filter(function(item) return item.file == file end, read_annotations(root))
  table.sort(matches, function(a, b) return (a.line or 1) < (b.line or 1) end)
  if #matches == 0 then return end

  local target = matches[1]
  if direction < 0 then target = matches[#matches] end
...
function M.setup()
  vim.api.nvim_create_autocmd({ "BufEnter", "BufWritePost", "FocusGained" }, {
    ...
    callback = function(args) M.refresh_buffer(args.buf) end,
  })

  vim.api.nvim_create_user_command("Annotate", function(opts)
    local action = opts.fargs[1] or "add"
    if action == "add" then M.add_current()
    elseif action == "copy" then M.copy_current()
    elseif action == "copyall" then M.copy_all()
