...
local function build_header(picker_name)
	...
	local parts = {}
	if scope_keys[picker_name] then
		table.insert(parts, scope_keys[picker_name])
	end
	if util_keys[picker_name] then
	...
	-- fzf-lua main plugin
	{
			...
			local function navigate_history(direction)
				return function(_, opts)
					...
					if new_index < 1 or new_index > #dir_history then
						vim.notify(
							"At " .. (direction < 0 and "oldest" or "newest") .. " directory in history",
							vim.log.levels.WARN
						)
						return
					end
			...
			local function search_history_action(initial_scope)
				return function(_, opts)
					...
					elseif prompt_lower:match("buffer") then
						picker_type = "buffers"
					elseif
						prompt_lower:match("recent")
						or prompt_lower:match("oldfile")
						or prompt_lower:match("history")
					then
							...
							require("fzf-lua").fzf_exec(history, {
								...
								actions = {
									...
									["ctrl-e"] = function(selected)
										...
										if scope ~= "local" then
											vim.notify(
												"Can only edit local history. Switch to local mode (Ctrl-d) first.",
												vim.log.levels.WARN
											)
											return
										end
									...
									["ctrl-c"] = function()
										...
										if scope ~= "local" then
											vim.notify(
												"Can only clear local history. Switch to local mode (Ctrl-d) first.",
												vim.log.levels.WARN
											)
											return
										end
							...
							callback = function()
								...
								if self.preview_winid and current_win == self.preview_winid then
									...
									local function close_picker_from_preview()
										...
										if not cur_self then
											return
										end
										local fzf_buf = cur_self.fzf_bufnr
										if not fzf_buf or not vim.api.nvim_buf_is_valid(fzf_buf) then
											return
										end
										-- Confirm this is actually a terminal buffer (proves channel ownership)
										if vim.bo[fzf_buf].buftype ~= "terminal" then
											return
										end
					...
					branches = {
						...
						actions = {
							...
							["default"] = function(selected)
								...
								if not branch then
									vim.notify(
										"Could not extract branch name from: " .. selected[1],
										vim.log.levels.WARN
									)
									return
								end
		...
		keys = {
			...
			-- Buffer pickers
			{
				"<leader>fb",
				function()
					require("fzf-lua").buffers({ prompt = "Buffers (Local)> " })
				end,
			...
			},
			{
				"<leader>fB",
				function()
					require("fzf-lua").buffers({ prompt = "All Buffers> ", show_all_buffers = true })
				end,
				desc = "All Buffers",
			},
			...
			},
			{
				"<leader>fR",
				function()
					require("fzf-lua").oldfiles({ prompt = "Recent Files (Global)> " })
				end,
			...
			},
			{
				...
				function()
					require("fzf-lua").live_grep({
						rg_opts = "--column --line-number --no-heading --color=always --smart-case --glob '!*test*' --glob '!*spec*' --glob '!*.min.*'",
					})
				end,
			...
			-- Supports cross-picker selection: select a branch, switch to commits, select a commit
			{
				"<leader>gD",
				function()
					local fzf = require("fzf-lua")
					...
					local file_status_colors = [[perl -pe "s/^A\s+/\e[32mA\e[0m\t/; s/^M\s+/\e[33mM\e[0m\t/; s/^D\s+/\e[31mD\e[0m\t/; s/^[RC]\d*\s+/\e[36m$&\e[0m/;"]]
					local short_status_colors = [[perl -pe "s/^(\?\?)/\e[36m$1\e[0m/; s/^( A|A )/\e[32m$1\e[0m/; s/^( M|M |MM)/\e[33m$1\e[0m/; s/^( D|D )/\e[31m$1\e[0m/; s/^(R | R)/\e[36m$1\e[0m/;"]]
					local file_preview_commit = [[bash -c 'git show --no-patch --format="%C(yellow)commit %H%Creset%n%CblueAuthor:%Creset %an <%ae>%n%CblueDate:%Creset   %ad%n%n%Cgreen%B%Creset" --date=local --color=always "$1"; printf "\033[1;34m\nChanged files:\033[0m\n"; git diff-tree --no-commit-id --name-status -r "$1" | ]] .. file_status_colors .. [[' -- {1}]]
					local file_preview_branch = [[bash -c 'ref="$1"; [ "$ref" = "*" ] && ref="$2"; git log -1 --format="%C(yellow)commit %H%Creset%n%CblueAuthor:%Creset %an <%ae>%n%CblueDate:%Creset   %ad%n%n%Cgreen%B%Creset" --date=local --color=always "$ref"; printf "\033[1;34m\nChanged files vs HEAD:\033[0m\n"; git diff --name-status HEAD..."$ref" | ]] .. file_status_colors .. [[' -- {1} {2}]]
					local file_preview_stash = [[bash -c 'git show --no-patch --format="%C(yellow)stash %H%Creset%n%CblueAuthor:%Creset %an <%ae>%n%CblueDate:%Creset   %ad%n%n%Cgreen%B%Creset" --date=local --color=always "$1"; printf "\033[1;34m\nChanged files:\033[0m\n"; git stash show --name-status "$1" | ]] .. file_status_colors .. [[' -- {1}]]
					local file_preview_worktree = [[bash -c 'git -C "$1" log -1 --format="%C(yellow)commit %H%Creset%n%CblueAuthor:%Creset %an <%ae>%n%CblueDate:%Creset   %ad%n%n%Cgreen%B%Creset" --date=local --color=always; printf "\033[1;34m\nWorktree status:\033[0m\n"; git -C "$1" status --short | ]] .. short_status_colors .. [[' -- {1}]]

					-- Dynamic header showing current selection and worktree context
					local function get_header()
						local base =
							"CTRL-H (commits) ╱ CTRL-B (branches) ╱ CTRL-S (stashes) ╱ CTRL-W (worktrees) ╱ CTRL-X (clear) ╱ CTRL-/ (preview)"
						...
						elseif #selected_refs == 1 then
							return base
								...
								.. " ✓ "
								.. selected_refs[1]
								.. " (select another or Enter)"
						else
							return base .. "\n" .. context .. " ✓ " .. selected_refs[1] .. ".." .. selected_refs[2]
						end
					end

					-- Add ref to selection (avoids duplicates, max 2)
					local function add_ref(ref)
						for _, r in ipairs(selected_refs) do
							if r == ref then
								return
							end
						end
						table.insert(selected_refs, ref)
						if #selected_refs > 2 then
							table.remove(selected_refs, 1)
						end
					end
					...
					diffview_picker = function(picker_type)
						...
						local switch_actions = {
							...
							["ctrl-b"] = function()
								vim.schedule(function()
									diffview_picker("branches")
								end)
							end,
							["ctrl-s"] = function()
								vim.schedule(function()
									diffview_picker("stashes")
								end)
							end,
							["ctrl-w"] = function()
								vim.schedule(function()
									diffview_picker("worktrees")
								end)
							end,
							["ctrl-/"] = function()
								preview_hidden = not preview_hidden
								vim.schedule(function()
									diffview_picker(picker_type)
								end)
							end,
							["ctrl-x"] = function()
								selected_refs = {}
				...
				desc = "Diffview picker (cross-picker selection, Ctrl-H/B/W to switch)",
			},
		...
		keys = {
			{
				...
				function()
					...
					local function build_zoxide_cmd(scope, filter_path)
						...
						if scope == "Global" or not filter_path then
							-- Global: show all, just transform display
							return base_cmd .. " | sed 's|" .. home .. "/||g'"
						else
							-- Filter to paths under filter_path, then transform display
							-- Escape special regex chars in path
							local escaped_path = filter_path:gsub("([%.%-%+%[%]%(%)%$%^%%%?%*])", "\\%1")
							return base_cmd
								.. " | awk '$0 ~ /"
								.. escaped_path:gsub("/", "\\/")
								.. "/' | sed 's|"
					...
					local function make_scope_actions(launch_fn)
						return {
							...
							["alt-l"] = function(_, opts)
								local query = opts.__call_opts and opts.__call_opts.query or ""
								vim.schedule(function()
									launch_fn(query, "Local")
								end)
							end,
							["alt-p"] = function(_, opts)
								local query = opts.__call_opts and opts.__call_opts.query or ""
								vim.schedule(function()
									launch_fn(query, "Parents")
								end)
							end,
						}
					end
					...
					local function launch_zoxide_picker(initial_query, scope)
						...
						local fzf = require("fzf-lua")
						local header =
							"--header='M-g: global │ M-s: git │ M-l: local │ M-p: parents │ Tab: drill down │ Enter: cd'"
						...
						fzf.zoxide({
							...
							query = initial_query or "",
							cmd = cmd,
							preview = 'bash -c \'path=$(echo {2..} | xargs); [[ "$path" != /* ]] && path="'
								.. home
								.. '/$path"; '
								.. preview_cmd
								.. ' "$path"\'',
