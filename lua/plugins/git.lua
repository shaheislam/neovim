	...
	-- Diffview for comprehensive git diff and merge conflict resolution
	{
		...
		keys = {
			...
			{ "<leader>gq", "<cmd>DiffviewClose<cr>", desc = "Close Diffview" },
			{
				"<leader>gD",
				function()
					-- Use fzf-lua to pick two commits/branches and open DiffviewOpen
					-- Ctrl-B: switch to branches, Ctrl-H: switch to commits
					local fzf = require("fzf-lua")
					local base_ref = nil
					local current_phase = "base" -- "base" or "compare"

					-- Common actions for view switching
					local function make_actions(on_select)
						return {
							["default"] = on_select,
							["ctrl-b"] = function()
								-- Switch to branches view
								vim.schedule(function()
									if current_phase == "base" then
										pick_base_branch()
									else
										pick_compare_branch()
									end
					...
					-- Extract ref from selection (handles both commit hashes and branch names)
					local function extract_ref(selected)
						if not selected or #selected == 0 then
							return nil
						end
						local item = selected[1]
						-- Try commit hash first (starts with hex)
						local hash = item:match("^(%x+)")
						if hash then
							return hash
						end
						-- For branches, extract the branch name (handles remotes/origin/branch format)
						local branch = item:match("^%*?%s*([^%s]+)") or item:match("^%s*remotes/[^/]+/([^%s]+)")
						return branch
					end

					-- Phase 1: Pick BASE (commits)
					function pick_base_commit()
						current_phase = "base"
						fzf.git_commits({
							prompt = "BASE (commits)> ",
							fzf_opts = { ["--header"] = "ctrl-h: commits │ ctrl-b: branches" },
							actions = make_actions(function(selected)
								base_ref = extract_ref(selected)
								if base_ref then
									vim.schedule(pick_compare_commit)
								end
							end),
						})
					end

					-- Phase 1: Pick BASE (branches)
					function pick_base_branch()
						current_phase = "base"
						fzf.git_branches({
							prompt = "BASE (branches)> ",
							fzf_opts = { ["--header"] = "ctrl-h: commits │ ctrl-b: branches" },
							actions = make_actions(function(selected)
								base_ref = extract_ref(selected)
								if base_ref then
									vim.schedule(pick_compare_branch)
								end
							end),
						})
					end

					-- Phase 2: Pick COMPARE (commits)
					function pick_compare_commit()
						current_phase = "compare"
						fzf.git_commits({
							prompt = "COMPARE (commits) [base: " .. base_ref .. "]> ",
							fzf_opts = { ["--header"] = "ctrl-h: commits │ ctrl-b: branches" },
							actions = make_actions(function(selected)
								local compare_ref = extract_ref(selected)
								if compare_ref then
									vim.cmd("DiffviewOpen " .. base_ref .. ".." .. compare_ref)
								end
							end),
						})
					end

					-- Phase 2: Pick COMPARE (branches)
					function pick_compare_branch()
						current_phase = "compare"
						fzf.git_branches({
							prompt = "COMPARE (branches) [base: " .. base_ref .. "]> ",
							fzf_opts = { ["--header"] = "ctrl-h: commits │ ctrl-b: branches" },
							actions = make_actions(function(selected)
								local compare_ref = extract_ref(selected)
								if compare_ref then
					...
					-- Start with commits view
					pick_base_commit()
				end,
				desc = "Compare commits/branches (fzf picker)",
			},
		},
		config = function()
