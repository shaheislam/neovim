...
emit("OpencodeEvent:agent.status", { sessionID = "ses-bound", status = { type = "question" } })
eq(last_async_argv()[6], "question", "an exact question state is published without collapsing to busy")
finish_one()
emit("OpencodeEvent:agent.status", { sessionID = "ses-bound", status = { type = "permission" } })
eq(last_async_argv()[6], "permission", "an exact permission state is preserved")
finish_one()
emit("OpencodeEvent:agent.status", { sessionID = "ses-bound", status = { type = "error" } })
eq(last_async_argv()[6], "error", "an exact error state is preserved")
finish_one()
emit("OpencodeEvent:agent.status", { sessionID = "ses-bound", status = { type = "busy" } })
eq(last_async_argv()[6], "busy", "a later exact busy state clears the higher-priority state")
finish_one()
...
emit("OpencodeEvent:agent.status", { sessionID = "ses-new", status = { type = "question" } })
eq(last_async_argv()[6], "question", "question outranks busy across exact bound sessions")
finish_one()
emit("OpencodeEvent:agent.status", { sessionID = "ses-b", status = { type = "permission" } })
eq(last_async_argv()[6], "permission", "permission outranks question across exact bound sessions")
finish_one()
emit("OpencodeEvent:agent.status", { sessionID = "ses-new", status = { type = "error" } })
eq(last_async_argv()[6], "error", "error outranks permission across exact bound sessions")
finish_one()

emit("OpencodeEvent:agent.status", { sessionID = "ses-new", status = { type = "idle" } })
eq(last_async_argv()[6], "permission", "lower-priority idle reveals the remaining permission state")
finish_one()
emit("OpencodeEvent:agent.status", { sessionID = "ses-b", status = { type = "idle" } })
eq(last_async_argv()[6], "idle", "all exact bound sessions can return to idle")
finish_one()
